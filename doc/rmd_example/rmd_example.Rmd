...
```{r horse-pops-plot, out.width="100%", fig.cap = "Horse populations for all provinces in Canada from 1906 - 1972"}
...
```{r horses-sd-table, out.width="50%"}
...
```{r horse-largest-sd, out.width="60%", fig.cap = "Horse populations for the province with the largest standard deviation"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In Figure \@ref(fig:horse-largest-sd) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
